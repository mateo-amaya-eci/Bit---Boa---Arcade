...
#define API_KEY "AIzaSyCRCUX_OFNxMOBAlgGM4yClNEgIgjpSTZw"
...
void enviarPuntaje() {
  if (datosEnviados) return; // Evita mandar el mismo puntaje mil veces

  Serial.println("Conectando a Wi-Fi...");
  WiFi.begin(WIFI_SSID, WIFI_PASSWORD);
  
  unsigned long tiempoInicio = millis();
  // Esperar máximo 10 segundos a que conecte
  ...
  if (WiFi.status() == WL_CONNECTED) {
    ...
    // Creamos el JSON con el nombre del usuario y su puntaje
    FirebaseJson json;
    json.set("usuario", String(us));
    json.set("puntos", puntos);

    Serial.println("Enviando puntos a Firebase...");
    // Usamos "push" para que cada partida sea un registro nuevo en el ranking
    if (Firebase.RTDB.pushJSON(&fbdo, "/ranking", &json)) {
      Serial.println("¡Puntaje guardado con éxito!");
      datosEnviados = true;
    } else {
      Serial.println("Error al enviar: " + fbdo.errorReason());
    }
    
    WiFi.disconnect(); // Desconectamos para ahorrar energía y volver al modo juego
  } else {
